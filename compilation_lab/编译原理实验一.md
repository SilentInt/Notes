...
1. 先获取字符，判断当前字符属于哪种匹配，再从状态矩阵查找次态。
2. 为保证状态转换矩阵的可拓展性与简洁性，对部分情况在状态转换时进行特判处理。

- 若现态为D2和E2，要对科学计数法进行处理，判断当前字符是否为e或E，
...
![文本  描述已自动生成](./assets/clip_image014.jpg)

wtype用于存放单词类型，value指针用于存放值的地址

next指针用于存放下一个单词的地址。

![文本  描述已自动生成](./assets/clip_image016.jpg)
...
![文本  描述已自动生成](./assets/clip_image022.jpg)

实验结果如下：

![文本  描述已自动生成](./assets/clip_image024.jpg)
