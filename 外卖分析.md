...
未定义返回参数的不要求有效返回数据，用`[]`包裹的为列表，用`()`包裹的是可选参数，`...`代表相同类型的参数多个，`{}`包裹的为嵌套json
...
{page, pagesize, (name)}
->
{id, cid, cover, price, status, discount, descr}
# 批量删除菜品
[菜品id ...]
# 批量停售
[菜品id ...]
...
{id}
->
{店铺id, cover, name, phone, status, address, announce}
# 店铺信息修改(按需更新)
{...}
```
